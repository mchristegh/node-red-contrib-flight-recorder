...
function fallbackClone(value, seen) {
  ...
  if (Array.isArray(value)) {
    const out = [];
    seen.set(value, out);
    for (let i = 0; i < value.length; i++) out[i] = fallbackClone(value[i], seen);
    return out;
  }
  const out = {};
  seen.set(value, out);
...
function payloadTypeOf(encodedMsg) {
  if (!Object.prototype.hasOwnProperty.call(encodedMsg, "payload")) return "undefined";
...
function buildSummary(encodedMsg) {
  ...
  if (type === "buffer") {
    ...
    payloadBytes = typeof Buffer !== "undefined"
      ? Buffer.byteLength(p, "utf8") : p.length;
    previewSource = p;
...
function createNormalizer(config, deps) {
  ...
  if (cfg.captureMode !== "full" && cfg.captureMode !== "summary") {
    throw new Error("normalize: captureMode must be 'full' or 'summary', got '" + cfg.captureMode + "'");
  }
  ...
  if (!Array.isArray(cfg.redactPaths) || cfg.redactPaths.some(p => typeof p !== "string" || p.length === 0)) {
    throw new Error("normalize: redactPaths must be an array of non-empty strings");
  }
  ...
  function normalize(msg) {
    ...
    let source = msg;
    if (Object.prototype.hasOwnProperty.call(msg, "req") ||
        Object.prototype.hasOwnProperty.call(msg, "res")) {
      source = Object.assign({}, msg);
      if (Object.prototype.hasOwnProperty.call(source, "req")) source.req = HTTP_REQ_MARK;
      if (Object.prototype.hasOwnProperty.call(source, "res")) source.res = HTTP_RES_MARK;
    }
...
module.exports = {
  ...
  _internal: {
    ...
    markers: {
      REDACTED, CIRCULAR, FUNCTION_MARK, HTTP_REQ_MARK, HTTP_RES_MARK
    }
  }
};
