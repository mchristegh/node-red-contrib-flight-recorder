...
class RecorderStore extends EventEmitter {
  constructor(storeId, config, deps) {
    ...
    cfg.persistDebounce = Object.assign({}, CONFIG_DEFAULTS.persistDebounce,
    ...
    if (!(Number.isFinite(cfg.incidentCooldownMs) && cfg.incidentCooldownMs >= 0)) {
    ...
    this._setTimeout = typeof d.setTimeout === "function" ? d.setTimeout : setTimeout;
    this._clearTimeout = typeof d.clearTimeout === "function" ? d.clearTimeout : clearTimeout;
    this._persistence = d.persistence || null;
    this._log = d.log || { warn: () => {}, error: () => {} };
    this._encode = typeof d.encode === "function" ? d.encode : _internal.fallbackEncode;
  ...
  detach(ref, opts) {
    ...
    if (this.config.persist && this._persistence) {
      if (opts && opts.removed) {
        try { this._persistence.remove(this.storeId); }
  ...
  // =========================================================================
  capture(input) {
    const at = (input && Number.isFinite(input.capturedAt)) ? input.capturedAt : this._now();
    ...
    return { accepted: true, seq: seq, dropped: false, truncated: normalized.truncated };
  }
  ...
  dump(trigger, context) {
    ...
    if (trigger !== "manual" && this.config.incidentCooldownMs > 0 &&
        this._lastAutoIncidentAt !== null) {
  ...
  resume() {
    if (this.state === "recording") return { state: "recording", changed: false };
  ...
  _writeState(reason) {
    ...
    try {
      ...
      const done = () => this.emit("persisted", {
        at: state.savedAt, recordCount: this.records.length, reason: reason
      });
  ...
  _restoreState() {
    ...
    try {
      ...
      this._log.warn("flight-recorder: persist file unreadable for '" +
        this.storeId + "', starting fresh (" + e.message + ")");
      return;
    }
    ...
    try {
      if (saved.formatVersion !== FORMAT_VERSION) {
        this._log.warn("flight-recorder: persist formatVersion " +
          saved.formatVersion + " unsupported for '" + this.storeId +
          "', starting fresh");
        return;
      }
      const records = Array.isArray(saved.records) ? saved.records : [];
      this.records = records.map((r) => Object.assign({}, r, { restored: true }));
      this.seqCounter = Number.isFinite(saved.seqCounter)
        ? Math.max(saved.seqCounter, 0) : 0;
      this.state = saved.state === "paused" ? "paused" : "recording";
      if (saved.stats && typeof saved.stats === "object") {
        if (Number.isFinite(saved.stats.droppedWhilePaused)) this.droppedWhilePaused = saved.stats.droppedWhilePaused;
        if (Number.isFinite(saved.stats.wrapCount)) this.wrapCount = saved.stats.wrapCount;
        if (Number.isFinite(saved.stats.truncatedCount)) this.truncatedCount = saved.stats.truncatedCount;
      }
      const n = this.records.length;
      ...
      this._log.warn("flight-recorder: persist file corrupt for '" +
        this.storeId + "', starting fresh (" + e.message + ")");
      this.records = [];
      this.restoredFromPersist = false;
    }
