...
const { createFilePersistence } = require(
...
function makeWatermark(property, operator, rawValue) {
  ...
  return function evaluate(msg) {
    ...
    switch (operator) {
      case "eq":
        return bothNumeric ? an === en : String(actual) === String(rawValue);
      case "neq":
        return bothNumeric ? an !== en : String(actual) !== String(rawValue);
      case "gt":
        return bothNumeric && an > en;
      case "gte":
        return bothNumeric && an >= en;
      case "lt":
        return bothNumeric && an < en;
      ...
      case "contains":
        try {
          return String(actual).indexOf(String(rawValue)) !== -1;
        } catch {
          return false;
        }
      default:
...
function makePersistenceAdapter(RED, node, wantPersist) {
  ...
  if (!userDir) {
    node.warn(
      "flight-recorder: persistence enabled but no userDir available; persistence disabled",
    );
    return null;
  }
