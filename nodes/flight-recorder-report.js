...
const { _internal: reportInternal } = require(
...
function sanitizeToken(v) {
  let s = String(v === undefined || v === null ? "" : v).replace(
    /[^A-Za-z0-9._-]/g,
    "_",
  );
  s = s.replace(/^\.+/, "_");
...
function resolveTokens(template, payload, kind, format) {
  const at =
    kind === "incident" && Number.isFinite(payload.triggeredAt)
      ? payload.triggeredAt
      : Date.now();
  ...
  const tokens = {
    ...
    incidentId:
      kind === "incident"
        ? String(payload.incidentId || "").slice(0, 8)
        : "status",
    format: format,
  };
  return template.replace(
    /\{(date|time|trigger|storeId|incidentId|format)\}/g,
...
module.exports = function (RED) {
  function FlightRecorderReportNode(n) {
    ...
    const format =
      n.format === "markdown" || n.format === "html" ? n.format : "text";
    const destination =
      n.destination === "file" || n.destination === "both"
        ? n.destination
        : "message";
    const directory = typeof n.directory === "string" ? n.directory.trim() : "";
    const template =
      typeof n.filenameTemplate === "string" && n.filenameTemplate.trim().length
        ? n.filenameTemplate.trim()
        : DEFAULT_TEMPLATE;
    ...
    if (destination === "message" && rawEnabled) {
      node.warn(
        "flight-recorder-report: raw JSON requires file output; disabled",
      );
      rawEnabled = false;
    }
    if ((fileEnabled || rawEnabled) && !directory) {
      node.warn(
        "flight-recorder-report: file output requested but no directory configured; file output disabled",
      );
      fileEnabled = false;
      rawEnabled = false;
    }
    ...
    function applyRetention() {
      ...
      const ordered = [...stems.entries()].sort(
        (a, b) => b[1].mtime - a[1].mtime || (a[0] < b[0] ? 1 : -1),
      );
      for (const [, group] of ordered.slice(retention)) {
        for (const f of group.files) {
          try {
            ...
            node.warn(
              "flight-recorder-report: retention could not remove " + f,
            );
          }
        }
      }
    ...
    node.on("input", function (msg) {
      ...
      if (
        (r.kind === "incident" || r.kind === "snapshot") &&
        (fileEnabled || rawEnabled)
      ) {
        try {
      ...
      if (failed.length) {
        ...
        node.status({
          ...
          text:
            ...
            " ✓" +
            (files.length
              ? " · saved" + (files.length > 1 ? " ×" + files.length : "")
              : ""),
        });
      }
