...
function buildIncidentModel(inc, previewChars) {
  ...
  if (inc.meta && typeof inc.meta === "object") {
    metaLine = [inc.meta.package, inc.meta.nodeRed ? "Node-RED " + inc.meta.nodeRed : null,
...
function htmlIncident(m) {
  ...
  B.push("<div class=\"meta\">store <b>" + escapeHtml(m.storeId) + "</b> · incident <b>" +
    escapeHtml(m.idShort) + "</b> · " + escapeHtml(m.at) +
    (m.metaLine ? "<br>" + escapeHtml(m.metaLine) : "") + "</div>");
  B.push("</div>");
  if (m.cause) {
