...
function detect(msg) {
  ...
  if (p && typeof p === "object") {
    if (typeof p.incidentId === "string" && Array.isArray(p.records))
      return "incident";
    if (
      p.stats &&
      p.config &&
...
function recordPreview(record, previewChars) {
  ...
  let base;
  if (m.payloadType !== undefined) {
    // summary form (config or truncation)
    base =
      m.payloadPreview !== undefined
        ? String(m.payloadPreview)
        : "(" + m.payloadType + ")";
  ...
  return base.length > previewChars
    ? base.slice(0, previewChars - 1) + "…"
    : base;
}
...
function buildIncidentModel(inc, previewChars) {
  ...
  const gaps = records
    .filter((r) => r.annotation === null || r.annotation === undefined)
    .map((r) => r.deltaMs)
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    if (r.annotation !== null && r.annotation !== undefined) {
      ...
      rows.push({
        kind: "mark",
        label: label,
        text: compactJson(r.annotation, previewChars + 20),
        seq: r.seq,
      });
    } else {
      rows.push({
        ...
        anom:
          med > 0 &&
          Number.isFinite(r.deltaMs) &&
          r.deltaMs >= ANOMALY_FACTOR * med,
  ...
  if (inc.trigger === "error" && ctx && typeof ctx === "object" && ctx.error) {
    ...
    cause = {
      message:
        e && typeof e === "object" && e.message !== undefined
          ? String(e.message)
          : compactJson(e, 120),
  ...
  const s = inc.stats || {};
  let metaLine = null;
  if (inc.meta && typeof inc.meta === "object") {
    metaLine = [
      inc.meta.package,
      inc.meta.nodeRed ? "Node-RED " + inc.meta.nodeRed : null,
      inc.meta.node ? "node " + inc.meta.node : null,
      inc.meta.hostname ? "host " + inc.meta.hostname : null,
    ]
      .filter(Boolean)
      .join(" · ");
  }
  return {
    ...
    stats: {
      ...
      spanText: humanize(
        s.spanMs === null || s.spanMs === undefined ? NaN : s.spanMs,
      ),
...
function buildSnapshotModel(q) {
  ...
  const model = {
    ...
    counters:
      ...
      (s.truncatedCount || 0) +
      " · dropped " +
      (s.droppedWhilePaused || 0),
    configLine:
      (c.captureMode === "summary" ? "summary" : "full") +
      " capture · " +
      c.maxRecordBytes +
      " B max · cooldown " +
  ...
  if (q.tap && typeof q.tap === "object") {
    ...
    model.tapLine =
      "auto-error " +
      (q.tap.autoErrorDump ? "ON" : "off") +
      " · mutes: " +
      (mutes.length ? mutes.join(", ") : "none") +
      " · seen " +
      (seen.inScope || 0) +
      " in / " +
      (seen.outOfScope || 0) +
...
function textIncident(m, previewChars) {
  ...
  L.push(" at         " + m.at);
  if (m.metaLine) L.push(" runtime    " + m.metaLine);
  if (m.cause) {
  ...
  L.push("─".repeat(W));
  L.push(
    " ⚠ gap ≥ " + ANOMALY_FACTOR + "× median · times relative to first record",
  );
  return L.join("\n");
}
...
function mdIncident(m) {
  ...
  if (m.metaLine) L.push("<br>*" + m.metaLine + "*");
  L.push("");
  if (m.cause) {
    L.push(
      "> **cause** " +
...
function htmlIncident(m) {
  ...
  B.push(
    "<h1>Flight Recorder Incident · " +
      escapeHtml(m.trigger.toUpperCase()) +
      "</h1>",
  );
  B.push(
    '<div class="meta">store <b>' +
      escapeHtml(m.storeId) +
      "</b> · incident <b>" +
      escapeHtml(m.idShort) +
      "</b> · " +
      escapeHtml(m.at) +
      (m.metaLine ? "<br>" + escapeHtml(m.metaLine) : "") +
      "</div>",
  );
  B.push("</div>");
  if (m.cause) {
    B.push(
      '<div class="cause"><b>cause:</b> ' +
        escapeHtml(m.cause.message) +
        (m.cause.source
          ? "<br><b>from:</b> " + escapeHtml(m.cause.source)
          : "") +
        "</div>",
    );
  } else if (m.contextText) {
  ...
  B.push(
    '<div class="stats">' +
      m.stats.recordCount +
      " records · span " +
      escapeHtml(m.stats.spanText) +
      " · median gap " +
      escapeHtml(m.stats.medianText) +
      " · wraps " +
      m.stats.wraps +
      " · truncated " +
      m.stats.truncated +
  ...
  if (m.rows.length === 0) {
    ...
    B.push(
      "<table><tr><th>seq</th><th>+time</th><th>gap</th><th>source</th><th>payload</th></tr>",
    );
    for (const row of m.rows) {
      if (row.kind === "record") {
        B.push(
...
function htmlSnapshot(m) {
  ...
  B.push(
    '<div class="meta">store <b>' + escapeHtml(m.storeId) + "</b></div></div>",
  );
  B.push(
    '<div class="stats">state <b>' +
      escapeHtml(m.state) +
      "</b> · restored " +
      (m.restored ? "yes" : "no") +
      "<br>" +
      escapeHtml(m.recording) +
      "<br>" +
      escapeHtml(m.counters) +
      "<br>" +
      escapeHtml(m.configLine) +
      (m.tapLine ? "<br>tap: " + escapeHtml(m.tapLine) : "") +
      "</div>",
  );
...
function render(msg, opts) {
  const o = opts || {};
  const format =
    o.format === "markdown" || o.format === "html" ? o.format : "text";
  const previewChars =
    Number.isFinite(o.previewChars) && o.previewChars > 0
      ? Math.floor(o.previewChars)
      : 60;
