...
function lastStatus(node) {
  return node.statusCalls[node.statusCalls.length - 1];
}
...
(async function main() {
  ...
  // =========================================================================
  {
    ...
    check(
      "I1 registered",
      h !== null && h.kind === "inline" && typeof h.runCommand === "function",
    );
    check(
      "I1 initial status",
      lastStatus(node).fill === "green" && lastStatus(node).text === "0/10",
  ...
  // =========================================================================
  {
    ...
    check(
      "I2 passthrough same reference",
      passed.length === 1 && passed[0] === msg,
    );
    check(
      "I2 msg unmutated",
      passed[0].recorderEvent === undefined &&
        JSON.stringify(passed[0]) ===
          JSON.stringify({ _msgid: "m1", topic: "t", payload: { v: 1 } }),
    );
    check(
      "I2 captured",
    ...
    check(
      "I2 source is this node",
      node.recorderStore.records[0].source.type === "flight-recorder",
    );
    await node.close(false);
  }
  ...
  // =========================================================================
  {
    ...
    node.receive({ payload: 2 });
    const ret = registry.get("i3-node").runCommand("query");
    const q = onOutput(node, 2);
    check(
      "I3 query on output 3",
      q.length === 1 &&
        q[0].recorderEvent === "query" &&
        q[0].payload.stats.recordCount === 2,
      JSON.stringify(q.map((m) => m.recorderEvent)),
    );
  ...
  // =========================================================================
  {
    ...
    const inc = registry
      .get("i4-node")
      .runCommand("dump", { requestedBy: "harness" });
    check(
      "I4 returned incident",
      inc.trigger === "manual" && inc.context.requestedBy === "harness",
    );
    const out2 = onOutput(node, 1);
    check(
      "I4 incident output",
      out2.length === 1 &&
        out2[0].payload.incidentId === inc.incidentId &&
        out2[0].topic === "my recorder",
    );
    const copies = events(node, "incident");
    check(
      "I4 events copy",
      copies.length === 1 && copies[0].payload.incidentId === inc.incidentId,
    );
    await node.close(false);
  }
  ...
  // =========================================================================
  {
    ...
    check(
      "I5 passthrough continued while paused",
      onOutput(node, 0).length === 4 && onOutput(node, 0)[1] === m,
    );
    ...
    check(
      "I5 paused/resumed events",
      events(node, "paused").length === 1 &&
        events(node, "resumed").length === 1,
    );
    check(
      "I5 redundant pause ignored-ack",
      r2.changed === false &&
    ...
    check(
      ...
      node.statusCalls.some(
        (s) => s.fill === "yellow" && /paused \(/.test(s.text),
      ),
      JSON.stringify(node.statusCalls.map((s) => s.text)),
    );
    check(
      "I5 status recovered",
      lastStatus(node).fill === "green" && lastStatus(node).text === "2/10",
      JSON.stringify(lastStatus(node)),
    );
    await node.close(false);
  }
  ...
  // =========================================================================
  {
    ...
    check(
      "I6 watermark incident",
      incs.length === 1 && incs[0].payload.trigger === "watermark",
    );
    check(
      "I6 context describes condition",
      incs[0].payload.context.condition.property === "payload.temp" &&
        incs[0].payload.context.matchedSeq === 2,
      JSON.stringify(incs[0].payload.context),
    );
    check(
      "I6 matching msg on the tape",
      incs[0].payload.records.length === 2 &&
  ...
  // =========================================================================
  {
    const node = makeNode(RED, {
      ...
      watermarkProperty: "payload",
      watermarkOperator: "eq",
      watermarkValue: "hot",
    });
    ...
    check(
      "I7 suppression visible",
      sup.length === 1 && sup[0].payload.trigger === "watermark",
    );
    check(
      "I7 manual still fires",
      registry.get("i7-node").runCommand("dump") !== null &&
  ...
  // =========================================================================
  {
    ...
    node.receive({
      payload: { password: "p", token: "t", apikey: "k", ok: 1 },
    });
    const rec = node.recorderStore.records[0];
    check(
      "I8 all three masked",
      rec.msg.payload.password === "[REDACTED]" &&
        rec.msg.payload.token === "[REDACTED]" &&
        rec.msg.payload.apikey === "[REDACTED]",
    );
  ...
  // =========================================================================
  {
    ...
    check(
      "I9 circular captured",
      node.recorderStore.records.length === 1 &&
        node.recorderStore.records[0].msg.payload.self === "[Circular]",
    );
    check(
      "I9 mark command works",
      registry.get(node.id) === null ||
  ...
  // =========================================================================
  {
    ...
    check(
      "I10 restored",
      b.recorderStore.restoredFromPersist === true &&
        b.recorderStore.records.length === 2 &&
        b.recorderStore.records[0].msg.payload === "before restart",
    );
    check(
      "I10 restored status shown",
      lastStatus(b).text === "2/10",
      lastStatus(b).text,
    );
  ...
  // =========================================================================
  {
    ...
    RED2.settings = {};
    require.cache[
      require.resolve(path.join(__dirname, "..", "nodes", "flight-recorder.js"))
    ] &&
      delete require.cache[
        require.resolve(
          path.join(__dirname, "..", "nodes", "flight-recorder.js"),
        )
      ];
    ...
    check(
      "I11 warned",
      node.warned.length === 1 && /persistence disabled/.test(node.warned[0]),
    );
    check(
      "I11 still records",
      (node.receive({ payload: 1 }), node.recorderStore.records.length === 1),
    );
    check(
      "I11 persist off in store",
      node.recorderStore.config.persist === false,
    );
    await node.close(false);
  }
  ...
  // =========================================================================
  {
    ...
    check(
      "I12 meta present",
      inc.meta && typeof inc.meta === "object",
    ...
    check(
      "I12 hostname present",
      typeof inc.meta.hostname === "string" && inc.meta.hostname.length > 0,
    );
    await node.close(false);
  }
