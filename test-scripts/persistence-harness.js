...
const { createFilePersistence, sanitizeStoreId, SUBDIR } =
  require(path.join(__dirname, "..", "lib", "persistence.js"));
const { RecorderStore } =
...
// ===========================================================================
{
  ...
  const state = { formatVersion: 1, storeId: "abc123.def", seqCounter: 7,
    state: "recording", records: [{ seq: 7, msg: { payload: "hi" } }] };
  p.write("abc123.def", state);
  check("P1 dir created on write", fs.existsSync(storeDir));
  check("P1 file exists", fs.existsSync(path.join(storeDir, "abc123.def.json")));
...
// ===========================================================================
{
  ...
  fs.writeFileSync(path.join(storeDir, "corrupt1.json"), "{ not json !!", "utf8");
  let threw = false;
  try { p.read("corrupt1"); } catch (e) { threw = true; }
  check("P2 corrupt json throws", threw);

  // and the core turns that throw into warn-and-fresh (contract seam check)
  let warned = 0;
  const s = new RecorderStore("corrupt1", { persist: true }, {
    persistence: p, log: { warn: () => warned++, error: () => {} }
  });
  check("P2 core starts fresh on corrupt", s.records.length === 0 &&
        s.restoredFromPersist === false && warned === 1, warned);
}
...
// ===========================================================================
{
  check("P3 traversal neutralized", sanitizeStoreId("../../etc/passwd") === "_.._.._etc_passwd" ||
        sanitizeStoreId("../../etc/passwd").indexOf("/") === -1, sanitizeStoreId("../../etc/passwd"));
  check("P3 no leading dots", !/^\./.test(sanitizeStoreId("...hidden")));
  check("P3 normal id untouched", sanitizeStoreId("a1b2c3.d4e5f6") === "a1b2c3.d4e5f6");
  check("P3 length capped", sanitizeStoreId("x".repeat(500)).length === 128);
  let bad = false;
  try { sanitizeStoreId(""); } catch (e) { bad = true; }
  check("P3 empty rejected", bad);

  const p = createFilePersistence(userDir);
  p.write("../escape", { formatVersion: 1 });
  check("P3 hostile write stays inside dir",
        fs.existsSync(path.join(storeDir, sanitizeStoreId("../escape") + ".json")) &&
...
// ===========================================================================
{
  ...
  let threw = false;
  try { p.remove("gone-soon"); } catch (e) { threw = true; }
...
{
  const p = createFilePersistence(userDir);
  for (let i = 0; i < 20; i++) p.write("atomic-test", { formatVersion: 1, i: i });
  const leftovers = fs.readdirSync(storeDir).filter(f => f.indexOf(".tmp-") !== -1);
  check("P5 no temp leftovers", leftovers.length === 0, JSON.stringify(leftovers));
...
(async function P6() {
  const p = createFilePersistence(userDir);
  const mk = () => new RecorderStore("e2e-node-1", {
    capacity: 10, persist: true, persistDebounce: { captures: 2, ms: 60000 }
  }, { persistence: p, log: quietLog });

  // phase A: run, capture, mark, close
  const a = mk();
  a.attach("ref");
  a.capture({ msg: { payload: "first" }, sourceId: "n1", sourceType: "inject" });
  a.capture({ msg: { payload: "second" }, sourceId: "n1", sourceType: "inject" }); // debounce-count write
  ...
  check("P6 records back", b.records.length === 3 && b.records.every(r => r.restored === true));
  ...
  check("P6 downtime visible in delta", b.records[3].deltaMs >= 50, b.records[3].deltaMs);
