...
const { createNormalizer, _internal } = require(
  path.join(__dirname, "..", "lib", "normalize.js"),
);
...
// ---------------------------------------------------------------------------
{
  ...
  const msg = {
    _msgid: "abc123",
    topic: "sensors/temp",
    payload: { temp: 21.5, ok: true, tags: ["a", "b"] },
  };
  const r = n.normalize(msg);
  check(
    "T1 full round-trip deep-equal",
    JSON.stringify(r.msg) === JSON.stringify(msg),
    JSON.stringify(r.msg),
  );
  check("T1 truncated false", r.truncated === false);
  check(
    "T1 redacted empty",
    Array.isArray(r.redacted) && r.redacted.length === 0,
  );
  check(
    "T1 bytes positive finite",
    Number.isFinite(r.bytes) && r.bytes > 0,
    r.bytes,
  );
...
{
  const n = createNormalizer({
    redactPaths: ["payload.password", "payload.apikey", "headers.auth.token"],
  });
  ...
  check(
    "T2 present path masked",
    r.msg.payload.password === M.REDACTED,
    r.msg.payload.password,
  );
  check(
    "T2 present path listed",
    r.redacted.indexOf("payload.password") !== -1,
    JSON.stringify(r.redacted),
  );
  check(
    "T2 absent leaf not listed",
    r.redacted.indexOf("payload.apikey") === -1,
  );
  check(
    "T2 missing parent not listed",
    r.redacted.indexOf("headers.auth.token") === -1,
  );
  check(
    "T2 exactly one redaction",
    r.redacted.length === 1,
    JSON.stringify(r.redacted),
  );
...
// ---------------------------------------------------------------------------
{
  ...
  check(
    "T4 inner circular marker",
    r.msg.payload.self === M.CIRCULAR,
    JSON.stringify(r.msg.payload.self),
  );
...
// ---------------------------------------------------------------------------
{
  const n = createNormalizer({
    ...
    redactPaths: ["payload.password"],
  });
  const big = "x".repeat(1024 * 1024);
  const msg = {
    topic: "big/topic",
    payload: { password: "TOPSECRET99", blob: big },
  };
  const r = n.normalize(msg);
  check("T5 truncated true", r.truncated === true);
  check(
    "T5 summary shape",
    r.msg.payloadType === "object" &&
      typeof r.msg.payloadBytes === "number" &&
      typeof r.msg.payloadPreview === "string" &&
      Array.isArray(r.msg.msgKeys),
    JSON.stringify(Object.keys(r.msg)),
  );
  check(
    "T5 payloadBytes reflects original scale",
    r.msg.payloadBytes > 1024 * 1024,
    r.msg.payloadBytes,
  );
  check(
    "T5 preview capped at 120",
    r.msg.payloadPreview.length <= 120,
    r.msg.payloadPreview.length,
  );
  const whole = JSON.stringify(r);
  check(
    "T5 secret absent from entire output",
    whole.indexOf("TOPSECRET99") === -1,
  );
...
// ---------------------------------------------------------------------------
{
  ...
  const r = n.normalize(msg);
  check(
    "T6 buffer placeholder type",
    r.msg.payload && r.msg.payload.__type === "buffer",
    JSON.stringify(r.msg.payload),
  );
  check(
    "T6 length correct",
    r.msg.payload.length === 100000,
    r.msg.payload.length,
  );
  check(
    "T6 hex preview 32 bytes",
    r.msg.payload.preview === "ab".repeat(32),
    r.msg.payload.preview,
  );
...
// ---------------------------------------------------------------------------
{
  ...
  check(
    "T7 summary shape",
    r.msg.payloadType === "string" && r.msg.payloadPreview === "hello",
    JSON.stringify(r.msg),
  );
  ...
  check(
    "T7 msgKeys include _msgid",
    r.msg.msgKeys.indexOf("_msgid") !== -1,
    JSON.stringify(r.msg.msgKeys),
  );
}
...
// ---------------------------------------------------------------------------
{
  ...
  check(
    "T8 req placeholder",
    r.msg.req === M.HTTP_REQ_MARK,
    JSON.stringify(r.msg.req),
  );
  check(
    "T8 res placeholder",
    r.msg.res === M.HTTP_RES_MARK,
    JSON.stringify(r.msg.res),
  );
  check("T8 payload preserved", r.msg.payload === "body");
  check(
    "T8 original req untouched",
    msg.req === req && msg.req.method === "GET",
  );
...
// ---------------------------------------------------------------------------
{
  ...
  check(
    "T9 structured error",
    r.msg.error && r.msg.error.__type === "error",
    JSON.stringify(r.msg.error && r.msg.error.__type),
  );
  check(
    "T9 name/message",
    r.msg.error.name === "Error" && r.msg.error.message === "boom happened",
  );
  check(
    "T9 stack capped",
    typeof r.msg.error.stack === "string" && r.msg.error.stack.length <= 2000,
...
// ---------------------------------------------------------------------------
{
  ...
  const make = () => ({
    _msgid: "d1",
    topic: "t",
    payload: { key: "k", arr: [1, "two", null], nested: { a: 1 } },
  });
...
// ---------------------------------------------------------------------------
{
  ...
  const s = createNormalizer({ captureMode: "summary" }).normalize({
    topic: "t",
  });
  check(
    "E1 undefined payload type",
    s.msg.payloadType === "undefined",
    s.msg.payloadType,
  );
  ...
  check(
    "E2 undefined key dropped",
    !("gone" in r.msg.payload),
    JSON.stringify(r.msg.payload),
  );
  ...
  check(
    "E3 function marker",
    rf.msg.payload.fn === M.FUNCTION_MARK,
    JSON.stringify(rf.msg.payload.fn),
  );
  ...
  check(
    "E4 date iso",
    rd.msg.payload === "2026-07-12T00:00:00.000Z",
    rd.msg.payload,
  );
  ...
  let rn;
  try {
    rn = n.normalize("just a string");
  } catch {
    threw = true;
  }
  check(
    "E5 non-object msg tolerated",
    !threw && rn.msg.payload === "just a string",
  );

  // topic truncated to 256 in summary
  const longTopic = "t".repeat(1000);
  const rt = createNormalizer({ captureMode: "summary" }).normalize({
    topic: longTopic,
    payload: 1,
  });
  check(
    "E6 topic capped 256",
    rt.msg.topic.length === 256,
    rt.msg.topic.length,
  );
  ...
  let bad = false;
  try {
    createNormalizer({ captureMode: "verbose" });
  } catch {
    bad = true;
  }
  ...
  bad = false;
  try {
    createNormalizer({ maxRecordBytes: -1 });
  } catch {
    bad = true;
  }
