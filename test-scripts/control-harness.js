...
function makeRecorder(RED, cfg) {
  ...
  Ctor.call(node, Object.assign({
    ...
    name: "", capacity: "10", captureMode: "full", maxRecordBytes: "16384",
...
function recOut(node, idx) { return node.sent.map(a => a[idx]).filter(Boolean); }
// control output: single ack msgs
function acks(node) { return node.sent; }
function lastAck(node) { return node.sent[node.sent.length - 1]; }
...
(async function main() {
  ...
  // =========================================================================
  {
    ...
    ctl.receive({ _msgid: "cmd1", payload: "dump", context: { requestedBy: "ops" } });

    const incs = recOut(rec, 1);
    check("X1 incident on recorder", incs.length === 1 && incs[0].payload.trigger === "manual" &&
          incs[0].payload.context.requestedBy === "ops");
    const a = lastAck(ctl);
    check("X1 ack ok", a.payload.ok === true && a.payload.command === "dump" &&
          a.payload.targetId === "x1-rec", JSON.stringify(a.payload));
    check("X1 ack carries result", a.payload.result.incidentId === incs[0].payload.incidentId);
    check("X1 ack correlated", a.inReplyTo === "cmd1");
    check("X1 status success", ctl.statusCalls.some(s => s.fill === "green" && s.text === "dump ✓"));
    await rec.close(false); await ctl.close(false);
  }
  ...
  // =========================================================================
  {
    ...
    check("X2 ack", lastAck(ctl).payload.ok === true && lastAck(ctl).payload.command === "pause");
  ...
  // =========================================================================
  {
    ...
    check("X3 mark on tape", rec.recorderStore.records.length === 2 &&
    ...
    check("X3 cleared", rec.recorderStore.records.length === 0 &&
          lastAck(ctl).payload.result.cleared === 2);
    check("X3 recorder emitted cleared event", recOut(rec, 3).some(m => m.recorderEvent === "cleared"));
    await rec.close(false); await ctl.close(false);
  }
  ...
  // =========================================================================
  {
    ...
    rec.receive({ payload: 1 }); rec.receive({ payload: 2 });
    ctl.receive({ payload: "query" });
    const q = recOut(rec, 2);
    check("X4 snapshot on recorder query output", q.length === 1 &&
          q[0].payload.stats.recordCount === 2);
    check("X4 ack carries snapshot", lastAck(ctl).payload.result.stats.recordCount === 2);
    await rec.close(false); await ctl.close(false);
  }
  ...
  // =========================================================================
  {
    ...
    const a = lastAck(ctl);
    check("X5 nack", a.payload.ok === false && /unknown command/.test(a.payload.reason),
          JSON.stringify(a.payload));
    check("X5 warned", ctl.warned.length === 1);
    check("X5 red status", ctl.statusCalls.some(s => s.fill === "red"));
    check("X5 recorder untouched", rec.recorderStore.records.length === 0 &&
  ...
  // =========================================================================
  {
    ...
    check("X6 three nacks", acks(ctl).length === 3 &&
          acks(ctl).every(a => a.payload.ok === false &&
          /string payload/.test(a.payload.reason)));
  ...
  // =========================================================================
  {
    ...
    ctl.receive({ payload: "query" });
    check("X8 graceful after target close", lastAck(ctl).payload.ok === false &&
