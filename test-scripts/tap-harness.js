    ...
    remove(pattern) {
      if (pattern.startsWith("*.")) {
        ...
        for (const k of [...handlers.keys()])
          if (k.endsWith(suffix)) handlers.delete(k);
      } else {
        handlers.delete(pattern);
      }
    },
    dispatchSend(sendEvents) {
      for (const [k, fn] of handlers)
        if (k.startsWith("onSend.")) fn(sendEvents);
    },
    dispatchComplete(ev) {
...
function src(id, type, name, z) {
  return { id, type, name, z };
}
let msgSeq = 0;
function sendEv(source, msg, port, destId) {
  ...
  return {
    msg: msg,
    source: { id: source.id, node: source, port: port || 0 },
    destination: { id: destId || "dest" },
  };
}
function emit(RED, source, msg, port) {
  RED.hooks.dispatchSend([sendEv(source, msg, port)]);
}

// tap outputs: [incidents, query, events]
function tapOut(node, idx) {
  return node.sent
    .map((a) => (Array.isArray(a) ? a[idx] : null))
    .filter(Boolean);
}
function events(node, type) {
  return tapOut(node, 2).filter((m) => m.recorderEvent === type);
}
...
(async function main() {
  ...
  // =========================================================================
  {
    ...
    check(
      "S1 onSend hook registered",
      RED.hooks.names().some((k) => k === "onSend.flight-recorder-tap-s1a"),
    );
    check(
      "S1 no onComplete without autoError",
      !RED.hooks.names().some((k) => k.startsWith("onComplete.")),
    );
    check("S1 registry kind tap", registry.get("s1a").kind === "tap");
    await t1.close(false);

    const t2 = makeTap(RED, {
      id: "s1b",
      scopeFlows: "flow1",
      autoErrorDump: true,
    });
    check(
      "S1 onComplete with autoError",
      RED.hooks.names().some((k) => k === "onComplete.flight-recorder-tap-s1b"),
    );
    await t2.close(false);
    check(
      "S1 close removes all hooks",
      RED.hooks.count() === 0,
      RED.hooks.names().join(","),
    );
  }
  ...
  // =========================================================================
  {
    ...
    check(
      "S2 flow scope",
      tap.recorderStore.records.length === 2 &&
    ...
    check(
      "S2 lazy pickup of new node",
      tap.recorderStore.records.length === 3 &&
  ...
  // =========================================================================
  {
    ...
    check(
      "S3 selection minus exclusions",
      tap.recorderStore.records.length === 2 &&
        tap.recorderStore.records.map((r) => r.source.id).join(",") === "m1,h1",
    );

    const tap2 = makeTap(RED, {
      scopeAllFlows: true,
      excludeTypes: "http request",
    });
    emit(RED, http1, { payload: 5 });
    emit(RED, func1, { payload: 6 });
    check(
      "S3 all-flows minus excluded type",
      tap2.recorderStore.records.length === 1 &&
  ...
  // =========================================================================
  {
    ...
    check(
      "S4 recorders never record recorders",
      tap.recorderStore.records.length === 1 &&
  ...
  // =========================================================================
  {
    ...
    RED.hooks.dispatchSend([
      sendEv(func1, msg, 1, "d1"),
      sendEv(func1, msg, 1, "d2"),
      sendEv(func1, msg, 1, "d3"),
    ]);
    ...
    RED.hooks.dispatchSend([
      sendEv(func1, { _msgid: "a", payload: 1 }),
      sendEv(func1, { _msgid: "b", payload: 2 }),
    ]);
    check(
      "S5 distinct msgs both captured",
      tap.recorderStore.records.length === 3,
    );
    await tap.close(false);
  }
  ...
  // =========================================================================
  {
    ...
    check(
      "S6 suppressed not captured",
      recs.length === 5 && recs.map((r) => r.seq).join(",") === "1,2,3,4,5",
    );
    check("S6 mute mark on tape", recs[1].annotation.muted.value === "m1");
    check(
      "S6 unmute mark carries count",
      recs[3].annotation.unmuted.value === "m1" &&
        recs[3].annotation.suppressedCount === 2,
      JSON.stringify(recs[3].annotation),
    );
    check(
      "S6 muted/unmuted events",
      events(tap, "muted").length === 1 &&
        events(tap, "unmuted").length === 1 &&
        events(tap, "unmuted")[0].payload.suppressedCount === 2,
    );
    check(
      "S6 status showed mute",
      tap.statusCalls.some((s) => /1 muted/.test(s.text)),
    );
    ...
    check(
      "S6 type mute suppresses",
      tap.recorderStore.records.length === 6 /* 5 prior + mute mark */ &&
        q.tap.mutes.length === 1 &&
        q.tap.mutes[0].kind === "type" &&
        q.tap.mutes[0].suppressedCount === 2,
      JSON.stringify(q.tap.mutes),
    );
    check(
      "S6 scopeSeen in query",
      q.tap.scopeSeen.inScope >= 2,
      JSON.stringify(q.tap.scopeSeen),
    );
    check(
      "S6 redundant mute ignored",
      (tap.receive({ payload: "mute", target: { type: "mqtt in" } }),
      events(tap, "commandIgnored").some(
        (m) => m.payload.reason === "already muted",
      )),
    );
    await tap.close(false);
  }
  ...
  // =========================================================================
  {
    ...
    check(
      "S7 muteIgnored event",
      events(tap, "muteIgnored").length === 1 &&
  ...
  // =========================================================================
  {
    ...
    RED.hooks.dispatchComplete({
      msg: { payload: "bad" },
      node: { id: "f1", node: func1 },
      error: boom,
    });
    const incs = tapOut(tap, 0);
    check(
      "S8 error incident",
      incs.length === 1 && incs[0].payload.trigger === "error",
    );
    check(
      "S8 context structured",
      incs[0].payload.context.error.__type === "error" &&
        incs[0].payload.context.error.message === "division by cucumber" &&
        incs[0].payload.context.source.id === "f1",
      JSON.stringify(incs[0].payload.context.error),
    );
    check("S8 tape attached", incs[0].payload.records.length === 1);

    RED.hooks.dispatchComplete({
      msg: {},
      node: { id: "h1", node: http1 },
      error: boom,
    });
    ...
    check(
      "S8 muted failures still dump",
      tapOut(tap2, 0).length === 1 &&
        tapOut(tap2, 0)[0].payload.trigger === "error",
    );
    check(
      "S8 muted chatter still suppressed",
      tap2.recorderStore.records.filter((r) => r.annotation === null).length ===
        0,
    );
    await tap2.close(false);
  }
  ...
  // =========================================================================
  {
    const tap = makeTap(RED, {
      scopeAllFlows: true,
      watermarkProperty: "payload.temp",
      watermarkOperator: "gte",
      watermarkValue: "100",
    });
  ...
  // =========================================================================
  {
    ...
    check(
      "S11 non-string tolerated",
      events(tap, "commandIgnored").some((m) =>
        /string payload/.test(m.payload.reason),
      ),
    );
    check(
      "S11 commands not captured as traffic",
      tap.recorderStore.records.filter((r) => r.annotation === null).length ===
        1,
    );
    await tap.close(false);
  }
  ...
  // =========================================================================
  {
    ...
    check(
      "S12 mute via control ack",
      ctl.sent[0].payload.ok === true &&
        ctl.sent[0].payload.result.muted === true &&
        ctl.sent[0].payload.result.changed === true,
    ...
    check(
      "S12 unmute via control",
      ctl.sent[1].payload.result.suppressedCount === 1,
    );
    await tap.close(false);
  ...
  // =========================================================================
  {
    ...
    check(
      "S13 meta present",
      inc.meta &&
        /^node-red-contrib-flight-recorder@\d+\.\d+\.\d+$/.test(
          inc.meta.package,
        ),
