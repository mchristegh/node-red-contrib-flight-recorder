...
for (const type of Object.keys(RUNTIME_KEYS)) {
  ...
  check(
    label + " ports " + exp.inputs + "/" + exp.outputs,
    inputs &&
      Number(inputs[1]) === exp.inputs &&
      outputs &&
      Number(outputs[1]) === exp.outputs,
    JSON.stringify({
  ...
  const declared = [
    ...js.matchAll(/^\s*([A-Za-z0-9_]+)\s*:\s*\{\s*value:/gm),
  ].map((m) => m[1]);
  ...
  check(
    label + " defaults cover runtime keys",
    missing.length === 0,
    "missing: " + missing.join(","),
  );
  check(
    label + " no orphan defaults",
    extra.length === 0,
    "extra: " + extra.join(","),
  );
  ...
  const fieldIds = [
    ...template.matchAll(/id="node-input-([A-Za-z0-9_]+)"/g),
  ].map((m) => m[1]);
  const fieldsWithoutDefault = fieldIds.filter(
    (f) => declared.indexOf(f) === -1,
  );
  check(
    label + " every form field has a default",
    fieldsWithoutDefault.length === 0,
    fieldsWithoutDefault.join(","),
  );
  const defaultsWithoutField = declared.filter(
    (d) => fieldIds.indexOf(d) === -1,
  );
  check(
    label + " every default has a form field",
    defaultsWithoutField.length === 0,
...
// --- cross-file: command vocabulary in control help matches runtime ------------
{
  const help = fs.readFileSync(
    path.join(NODES_DIR, "flight-recorder-control.html"),
    "utf8",
  );
  const runtimeSrc = fs.readFileSync(
    path.join(NODES_DIR, "flight-recorder-control.js"),
    "utf8",
  );
  ...
  const undocumented = commands.filter(
    (c) => help.indexOf("<code>" + c + "</code>") === -1,
  );
  check(
    "control help documents all runtime commands",
